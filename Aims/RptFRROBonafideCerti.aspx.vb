...
Partial Class RptFRROBonafideCerti
    Inherits BasePage
    ...
    Protected Sub btnReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReport.Click
        If ChkBoxHeader.Checked = True Then
            ...
            If DDLBranch.SelectedItem.Value = "0" Or DDLA_Year.SelectedItem.Value = "0" Or DDLCourse.SelectedItem.Value = "0" Or DDLBatch.SelectedItem.Value = "0" Then
                ...
                DDLBranch.Focus()
            Else
                Dim qrystring As String = "RptFRROBonafideCertiV.aspx?" & QueryStr.Querystring() & "&BrID=" & BrID & "&AID=" & AID & "&CID=" & CID & "&BID=" & BID & "&country=" & country & "&Student=" & Student
        ...
        Else
            ...
            If DDLBranch.SelectedItem.Value = "0" Or DDLA_Year.SelectedItem.Value = "0" Or DDLCourse.SelectedItem.Value = "0" Or DDLBatch.SelectedItem.Value = "0" Then
                ...
                DDLBranch.Focus()
            Else
                Dim qrystring As String = "RptFRROBonafideCertiV1.aspx?" & QueryStr.Querystring() & "&BrID=" & BrID & "&AID=" & AID & "&CID=" & CID & "&BID=" & BID & "&country=" & country & "&Student=" & Student
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "message", "<script>window.open('" & qrystring & "',null,'toolbar=no,scrollbars=yes,location=no,resizable =yes,width=1050,height=700,left=0,top=0')</script>", False)
                msginfo.Text = ValidationMessage(1014)
            End If
        End If
