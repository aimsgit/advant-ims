...
Public Class Mfg_DLDebitNote
    ...
    Shared Function Insert(ByVal EL As Mfg_ELCreditNote) As Integer
        ...
        arParms(10) = New SqlParameter("@Debit", SqlDbType.VarChar, 10)
        arParms(10).Value = EL.Debit
        Try
            rowsAffected = SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, "Mfg_InsertCreditNote1", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return rowsAffected
    ...
    Shared Function Update(ByVal EL As Mfg_ELCreditNote) As Integer
        ...
        arParms(11) = New SqlParameter("@Debit", SqlDbType.VarChar, 10)
        arParms(11).Value = EL.Debit

        Try
            rowsAffected = SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, "Mfg_UpdateCreditNote1", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return rowsAffected
    ...
    Shared Function GetGridData(ByVal EL As Mfg_ELCreditNote) As Data.DataTable
        ...
        Dim ds As New DataSet
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Mfg_GetCreditNote1", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return ds.Tables(0)
