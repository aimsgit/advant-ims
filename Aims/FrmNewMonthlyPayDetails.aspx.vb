...
Partial Class FrmNewMonthlyPayDetails
    Inherits BasePage

    Dim m As New ELMonthlyPayDetails
    Dim mb As New BLMonthlyPayDetails
    Dim month As String
    Dim year, MonthNo As Integer
    Dim Dept As Integer
    ...
    Protected Sub btnGenerate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerate.Click
        If Session("Privilages").ToString.Contains("W") Then
            If (Session("BranchCode") = Session("ParentBranch")) Then
                ...
                dt = mb.duplicate(month, year, Dept, MonthNo)
                Try
                    If dt.Rows.Count > 0 Then
                        ...
                        Exit Sub
                    Else
                        mb.generate(month, year, Dept, MonthNo)
                        lblE.Text = ""
                    ...
                    For Each grid As GridViewRow In GridView1.Rows
                        ...
                        dt1 = mb.getgrid1(month, year, Dept, EmpId)
                        If dt1.Rows.Count > 0 Then
                            CType(grid.FindControl("txtPLDays"), TextBox).Text = dt1.Rows(0).Item("Paid").ToString
                            CType(grid.FindControl("txtLOP"), TextBox).Text = dt1.Rows(0).Item("NotPaid").ToString
                        End If
        ...
        If Session("Privilages").ToString.Contains("W") Then
            ...
            If (Session("BranchCode") = Session("ParentBranch")) Then
                Try
                    ...
                    lblS.Text = ""
                    For Each grid As GridViewRow In GridView1.Rows
                        m.MD_ID = CType(grid.FindControl("MD_ID"), HiddenField).Value.ToString
                        ...
                        If CType(grid.FindControl("txtLOP"), TextBox).Text = "" Then
                            m.LOP = "0.00"
                        Else
                            m.LOP = CType(grid.FindControl("txtLOP"), TextBox).Text
                        End If
                        mb.update(m)
