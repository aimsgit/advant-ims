...
Partial Class FrmGeneratesalaryNew
    Inherits BasePage
    Dim alt As String
    ...
    Protected Sub gvGenSalary_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles gvGenSalary.PageIndexChanging
        ...
        ViewState("PageIndex") = gvGenSalary.PageIndex
        GVFill()
    End Sub
    Protected Sub btnprint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnprint.Click
        If Session("Privilages").ToString.Contains("W") Or Session("Privilages").ToString.Contains("P") Then
            ...
            If ddlYear.SelectedItem.Text <> "" Then
                ...
                Prop.Year = ddlYear.SelectedItem.Text
                Prop.EMPid = 0
                dt = BLL.RptSalSlip2(Prop)
                If dt.Rows.Count <> 0 Then
                    Dim qrystring As String = "ReprtMySalarySlip.aspx?" & "&Month=" & ddlMonth.SelectedItem.Text & "&Year=" & ddlYear.SelectedItem.Text & "&EMPid=" & 0
    ...
    Protected Sub btnEditMonth_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEditMonth.Click
        If Session("Privilages").ToString.Contains("W") Then
            ...
            Dim qrystring As String = "FrmNewMonthlyPayDetails.aspx?"
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "message", "<script>window.open('" & qrystring & "',null,'toolbar=no,scrollbars=yes,location=no,resizable =yes,width=800,height=600,left=40,top=80')</script>", False)
    ...
    Sub GVFill()
        ...
        If ddlYear.SelectedItem.Text <> "" Then
            ...
            dt2 = BLL.RptSalSlip2(Prop)
            Prop.Lock = BLL.LockStatus1(Prop)
    ...
    Protected Sub BtnNegativeSalary_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNegativeSalary.Click
        If Session("Privilages").ToString.Contains("W") Or Session("Privilages").ToString.Contains("R") Then
            ...
            Prop.Year = ddlYear.SelectedItem.Text
            For Each rows As GridViewRow In gvGenSalary.Rows
                Prop.EMPid = CType(rows.FindControl("lblEmpId"), Label).Text
                dt = BLL.GetLoanDetails(Prop)
                If dt.Rows.Count > 0 Then
                    Dim i, j As Integer
                    i = 0
                    j = dt.Rows.Count
                    While j > 0
                        Prop.LoanCode = dt.Rows(i).Item("LoanCode").ToString()
                        Prop.LoanAmt = dt.Rows(i).Item("Monthlyded").ToString()
                        BLL.InsertLoan(Prop)
                        i = i + 1
                        j = j - 1
                    End While
            ...
            GVFill()
            lblmsg.Text = ""
            lblGreen.Text = "Loan updated sucessfully."
        Else
            lblmsg.Text = "You do not have Read Privilage."
        End If
    End Sub
