...
Partial Class Rpt_EthnicGenderWiseStudent
    Inherits BasePage
    ...
    Protected Sub BtnGReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnGReport.Click
        ...
        Stream1 = ddlView.SelectedItem.Text
        ...
        End If
        If ddlView.SelectedValue = 0 Then
            lblE.Text = "Select any View by."
            Exit Sub
        Else
            Stream = ddlView.SelectedValue
        End If
        ...
        End If
        If Stream = 1 Then
            Dim qrystring As String = "Rpt_EthnicGenderWiseStudentV.aspx?" & QueryStr.Querystring() & "&StartDate=" & fromdateE & "&EndDate=" & todateE & "&BranchCode=" & BranchCode & "&BranchCode1=" & BranchCode1 & "&Stream=" & Stream & "&Stream1=" & Stream1
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "message", "<script>window.open('" & qrystring & "',null,'toolbar=no,scrollbars=yes,location=no,resizable =yes,width=800,height=600,left=40,top=80')</script>", False)
        Else
            Dim qrystring As String = "Rpt_CompositionOfEthnicV.aspx?" & QueryStr.Querystring() & "&StartDate=" & fromdateE & "&EndDate=" & todateE & "&BranchCode=" & BranchCode & "&BranchCode1=" & BranchCode1 & "&Stream=" & Stream & "&Stream1=" & Stream1
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "message", "<script>window.open('" & qrystring & "',null,'toolbar=no,scrollbars=yes,location=no,resizable =yes,width=800,height=600,left=40,top=80')</script>", False)
        End If
    End Sub
End Class
