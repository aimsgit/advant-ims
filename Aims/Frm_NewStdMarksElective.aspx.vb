...
Partial Class Frm_NewStdMarksElective
    Inherits BasePage
    Dim el As New NewStdMarks
    Dim b As New BLStdMarksElective
    Dim d As New DLStdMarksElective
        ...
        If (Session("BranchCode") = Session("ParentBranch")) Then
            ...
            Try
                ...
                Else
                    ...
                    dt = DLNew_StudentMarks.GetBatch(el)
        ...
        If (Session("BranchCode") = Session("ParentBranch")) Then
            Try
                ...
                Else
                    ...
                    End If
                End If
                If ChkcalculateGradePoint.Checked = True Then
                    For Each grid As GridViewRow In GridView1.Rows
                        el.Grade = CType(grid.FindControl("txtgrade"), TextBox).Text
                        el.id = CType(grid.FindControl("lblid"), Label).Text
                        b.UpdateGradePoint(el)
                    Next
                    dt = b.ViewStdMarks(el)
                    GridView1.Visible = True
                    GridView1.DataSource = dt
        ...
        Try
            ...
            If dt3.Rows.Count <> 0 Then
                ...
                While count1 > 0
                    ' dt1.Rows(count - 1).Item("StudentCode")
                    count = dt.Rows.Count
                    If ChkGrade.Checked = False Then
                        While count > 0
                            If dt.Rows(count - 1).Item("StudentCode") = Trim(dt1.Rows(count1 - 1).Item("StdCode")) Then
                    ...
                    Else
                        While count > 0
                            If dt.Rows(count - 1).Item("StudentCode") = Trim(dt1.Rows(count1 - 1).Item("StdCode")) Then
                                dt1.Rows(count1 - 1).Item("Grade") = IIf(dt.Rows(count - 1).Item("Grade") = "", 0, dt.Rows(count - 1).Item("Grade"))
                                recordupdate = recordupdate + 1
                                Exit While
                            Else
                                count = count - 1
    ...
    Protected Sub btnparse_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnparse.Click
        ...
        Try
            ...
            Else
                Dim dcID = New DataColumn("StudentCode", GetType(String))
                Dim dcGrade = New DataColumn("Grade", GetType(String))
                dt.Columns.Add(dcID)
    ...
    Sub ViewDataStatus()
        Try
            ...
            el.Semester = ddlsemester.SelectedValue
            el.Subject = ddlsubject.SelectedValue
            Dim qrystring As String = "RptDataStatusV.aspx?" & QueryStr.Querystring() & "&Batch=" & el.Batch & "&Semester=" & el.Semester & "&Subject=" & el.Subject
    ...
    Protected Sub RptButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RptButton.Click
        Try
            ...
            Dim qrystring As String = "RptStudMarks.aspx?" & "&Ayear=" & el.A_Year & "&Batch=" & el.Batch & "&Semester=" & el.Semester & "&Subject=" & el.Subject & "&Assesment=" & el.Assesment & "&AssesmentDate=" & el.AssesmentDate & "&SubGrp=" & el.SubGrp & "&StdId=" & el.StdId
